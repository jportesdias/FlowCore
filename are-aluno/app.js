...
async function loadStudentState() {
  ...
  state.cursos = courseIds.map(courseId => {
    ...
    return {
      ...
      materiais: (remoteMaterials.length ? remoteMaterials : fallbackCourse?.materiais || [])
...
function renderCourse(id) {
  ...
  app.innerHTML = `
    ...
    <section class="video-shell is-hidden">
...
function renderMaterialItem(item) {
  ...
  if (item.tipo === "video") {
    return `
      <article class="material-item${doneClass}">
        <div>
...
function bindCompletionControls() {
  document.querySelectorAll("[data-completion-material]").forEach(input => {
    input.addEventListener("change", event => {
      ...
      setMaterialCompleted(materialId, event.currentTarget.checked);
      updateCourseProgress();
      renderRoute();
    });
  });
